...
def camber_stator(y0,Uhp,Uh,feh2,feh1,Vth1h,Vth2h,Wth1h,Wth2h):
    ...
    yRh_rotor = ys_cent + 0.75 * (Wth1h / Uh) * Uhp
    yLh_rotor = ys_cent - 0.75 * (Wth2h / Uh) * Uhp
...
def mid_camb_stator(U,fe,rne,pe,y0):
    ...
    return xs_cent, ys_cent, xRm_camb, yRm_camb, yLm_camb, xLm_camb

# def camber_rotor(y0,Uhp,Uh,feh2,feh1,Vth1h,Vth2h):
#     xs_cent = Window.size[0] * 200 / 400
#     ys_cent = y0 + Window.size[1] * 125 / 700
#     xRh_camb = xs_cent + 0.75 * Uhp * float(feh2)
#     yRh_camb = ys_cent - 0.75 * (Vth2h / Uh) * Uhp - Uhp
#     yLh_camb = ys_cent + 0.75 * (Vth1h / Uh) * Uhp - Uhp
#     xLh_camb = xs_cent - 0.75 * Uhp * float(feh1)
#
#     return xs_cent, ys_cent, xRh_camb, yRh_camb, yLh_camb, xLh_camb